...
__global__ void matrix_mul(const double* a, const double* b, double* c, int n) {
...
void export_bench_results(const BenchResult* results, int count) {
    ...
    std::ofstream file("results/bench_results9.txt");
    if (!file.is_open()) {
    ...
    for (int i = 0; i < count; ++i) {
        file << "B9|" << results[i].title << "|"
             << results[i].size << "|"
             << results[i].gpu_time << std::endl;
    }
    file.close();
    std::cout << "Results exported to bench_results9.txt" << std::endl;
}
...
int main() {
    ...
    for (int s = 0; s < ns_count; ++s) {
        ...
        for (int i = 0; i < n * n; ++i) {
            a[i] = fmod(i * 12345.6789, 1000.0);
            b[i] = fmod(i * 98765.4321, 1000.0);
        }
