...
void export_bench_results(const BenchResult* results, const int count) {
    ...
    std::ofstream file("results/bench_results3.txt");
    if (!file.is_open()) {
        std::cerr << "Error opening file for writing results." << std::endl;
        return;
    }
    // write as follows: B3|<bench_name>|<size>|<gpu_time>
    for (int i = 0; i < count; ++i) {
        file << "B3|" << results[i].title << "|"
             << results[i].size << "|"
             << results[i].gpu_time << std::endl;
    }
    file.close();
    std::cout << "Results exported to bench_results3.txt" << std::endl;
}

int main() {
    constexpr int max_iterations_list[] = {8, 16, 32, 64, 128, 256, 512, 1024, 2048};
    constexpr int max_iterations_list_count = sizeof(max_iterations_list) / sizeof(max_iterations_list[0]);
    BenchResult bench_results[max_iterations_list_count];
    ...
    for (int s = 0; s < max_iterations_list_count; ++s) {
        const int max_iterations = max_iterations_list[s];
        uint8_t *d_image;
        ...
        bench_results[s].size = max_iterations;
        bench_results[s].gpu_time = gpu_time;
        bench_results[s].title = std::to_string(pixel_span) + "x" + std::to_string(pixel_span);
    }

    stbi_write_png("./results/mandelbrot.png", pixel_span, pixel_span, 1, image.data(), pixel_span);

    export_bench_results(bench_results, max_iterations_list_count);
    return 0;
}
